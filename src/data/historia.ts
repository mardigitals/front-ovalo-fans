export // --- DATA HISTÓRICA DEL AUTÓDROMO ---
const ETAPAS_HISTORIA = [

  {

    id: 'etapa-1',

    fecha:'25 de mayo 1919',

    periodo: '1919 - 1925',

    titulo: 'El despertar de una pasión',

    resumen: 'Los primeros rugidos de velocidad en tierra a mas de 100 km/h.',

    descripcionLarga: `Año 1919, bautismo de nafta y aceite que signó una trayectoria para el automovilismo argentino, que convierte al Club Atlético de Rafaela en "Cumbre del automovilismo de América" que, sin ser los primeros, adquieren por su empeño y tenacidad un lugar de privilegio en la esfera automotriz. Su nacimiento ocurrió un gris 25 de mayo de 1919, en un circuito "chacarero" de 320 kilómetros de recorrido. Fue la primera competencia organizada por el Club Atlético de Rafaela. 

    Para que este hito fuera posible, un joven Atlético de Rafaela —que por entonces contaba con apenas 12 años de vida— conformó en el mes de febrero una Comisión de Carreras, antecesora de la actual Subcomisión de Automovilismo. Esta primera mesa  estuvo integrada por José Simonetta, Adolfo Bircher, Emilio Picasso, Federico Bollinger, Eduardo Ripamonti y Francisco Soldano.

    Estos dirigentes tuvieron la audaz y alocada idea de organizar una carrera de automóviles en una época donde las calles aún eran dominadas por sulkis, caballos y carretas. El automóvil estaba lejos de ser un medio de transporte masivo; de hecho, el primer vehículo a motor de la historia mundial, patentado por Benz, tenía poco más de 30 años de existencia.

    Fue así como, desafiando a su tiempo, el Día de la Patria marcó un antes y un después: se largó la primera carrera de automóviles en la ciudad con la participación de siete valientes pioneros: Ángel Gallé, Antonio Valenti, Juan Colombetti, Jorge Cohen, Juan Macci y Oberdán Piovano.

    El grupo de audaces pilotos se alineó sobre bulevar Lehmann y calle Alem, ese fue el lugar de la largada de un intenso recorrido de 320 km. que uniría Rafaela con las localidades de Lehmann, Ataliva, Sunchales, Tacural, Morteros, Brikmann, Porteña, Luxardo, San Francisco, Clucellas, Saguier y Susana. El vencedor de esa prueba histórica fue Oberdán Piovano con un coche Overland con el que realizó el recorrido en un tiempo de 4 horas, 9 minutos y 2 segundos, con un promedio de 77,349 km/h. Segundo fue Juan Colombetti con Studebaker y tercero Juan Macci con Chevrolet.

    Así fue el principio del automovilismo en Atlético de Rafaela. Una de las historias más ricas de este deporte en Argentina. En este proceso genuino, y siempre ejemplar, está la clave para comprender cómo fue posible que desde este club del interior del país, un buen día todo Indianápolis, sus pilotos, sus máquinas y hasta su propio estilo competitivo llegara a la Argentina, atraído por una convocatoria que no reconoce igual.

    Se continuó realizando hasta el año 1925, con distintos trazados por diferentes pueblos. 
    
    Este fue el punta pié inicial donde surgió la pasión del centro de la pampa gringa por los fierros, la que pronto destacaría al club y a la ciudad de Rafaela en los más altos conceptos y niveles del país y el mundo.`,

    fotos: [

      { url: '/images/modals/caballo-largada-1919.jpg', epigrafe: 'Grilla de largada en el centro de Rafaela.' },

      { url: '/images/modals/largada-1919.jpg', epigrafe: 'Largada en calle Alem y Bv. Lehmann, ambos caminos de tierra.' },

      { url: '/images/modals/catedral-rafaela-1919.jpg', epigrafe: 'Vista histórica de la 1 Carrera.' },

      { url: '/images/modals/cohen-1919.jpg', epigrafe: 'Corredor J. Cohen, en 1919.' },

      { url: '/images/modals/piovano-1919.jpg', epigrafe: 'O. Piovano, ganador de la histórica  Carrera.' },

      { url: '/images/modals/recorte-diario-1919.jpg', epigrafe: 'Recorte de diario alusivo a la primera Carrera.' }

    ]

  },

  {

    id: 'etapa-2',

    fecha:'6 de junio 1926',

    periodo: '1926',

    titulo: 'Primera "500 Millas Argentinas"',

    resumen: 'La consagración nacional y la mítica competencia en el óvalo de tierra.',

    descripcionLarga: `Amanecer en Rafaela

El Club Atlético de Rafaela y la ciudad toda se iniciaba en el quehacer automovilístico nacional al comenzar los trabajos para organizar nuevamente la carrera en 1926, se pensó crear las 500 Millas Argentinas.
El 3 de marzo se reunió la Comisión de Carreras, bajo la presidencia de Reinaldo Ripamonti, y llegaron a la decisión de rechazar la idea de correr las 500 Millas después de un largo debate, donde cada uno de los miembros expuso su pensamiento. Cuando la resolución del Club Atlético fue conocida y la prensa del país tomó conocimiento de la determinación del Club Atlético de no realizar las 500 Millas, los periodistas porteños, capitaneados por Antonio Casime, respaldado por Burgos Santillán y el chileno Emilio Karstulovic, anunciaban con grandes títulos, en los Diarios "La Razón" y "Crítica", que el Club Atlético de Rafaela había resuelto favorablemente la iniciativa de correr las "500 Millas Argentinas" a pesar de los problemas de luz y tiempo.
Por el alboroto que causó la noticia en los centros automovilísticos del país, se reunió la Comisión de Carreras y decidió dejar sin efecto su anterior disposición y se nombró una Comisión que viajó a Buenos Aires para sondear en los medios porteños la posibilidad de la competencia en su nueva modalidad, recabando la opinión de la prensa, directivos y corredores. Con la misión cumplida regresaron a Rafaela, donde toda la ciudad esperó con nerviosismo la reunión de la Comisión de Carreras. El cónclave se concretó el 27 de marzo de 1926, donde se estudiaron y se consideraron todas las posibilidades llegando a la siguiente resolución: se correrían por primera vez las "500 Millas Argentinas". 
El 27 de marzo en el libro de actas del club se deja asentado que: “estando todo dispuesto favorablemente se correrán las primeras 500 Millas Argentinas».

El 6 de junio de 1926 a las 7 de la mañana 29 autos hacían tronar el piso de tierra. Más de 40 mil personas esperaban la largada. La carrera se disputaría en un circuito de caminos vecinales, ubicado a 2.000 metros al Oeste de la hoy Ruta 34, en la prolongación del Bulevar Roca, Un dibujo de aproximadamente 37 Km que debía ser recorrido en 21 oportunidades y así encontrar una equivalencia gaucha de las 500 Millas Gringas en Indianápolis.

Se pusieron en marcha los cronómetros desde un vagón de tren que a un costado constituía un lugar preferencial para las autoridades de la competencia entre ellos los fiscalizadores enviados desde Buenos Aires por el ACA (Automóvil Club Argentino). Ruido de motores, gritos, aplausos, huellas, tierra por el aire, se largaron las 500 millas, la gran historia empezaba a escribirse. A las 2 horas de carrera se desató un temporal para que la competencia fuera más dramática en la lluvia y el barro. Domingo Bucci, quien figuró como el primer inscripto en la nómina se había situado en el liderazgo con su automóvil Hudson y ahí permaneció hasta que finalmente llegó la suspensión. El reloj marcaba 2 horas, 15 minutos y 1 segundo de carrera.

Pasarían semanas para que se retome la prueba deportiva, se produce una fuerte ruptura de relaciones entre Atlético y el Automóvil Club Argentino. La confusión ganó a muchos y las desprolijidades propias de la inexperiencia hicieron que se cometieran muchos errores al reanudar la prueba después de muchos intentos y postergaciones. Recién el 29 de agosto de 1926 se reanudan las 500 Millas Argentinas. Fue ese entonces el momento donde Raúl Riganti, piloto que llegó desde Buenos Aires y se convirtió en el memorable ganador del primer gran carrerón de la República Argentina.`,

    fotos: [

      { url: '/images/modals/largada-1926.jpg', epigrafe: 'Largada histórica de la 1 500 Millas Argentinas.' },

      { url: '/images/modals/llegada-1926.jpg', epigrafe: 'Bandera a cuadros para Riganti.' }

    ]

  },

  
  {

    id: 'etapa-3',

    fecha:'8 de septiembre 1935',

    periodo: '1935',

    titulo: '500 Millas, con el apoyo del ACA',

    resumen: 'El retorno de las 500, luego de la crisis económica.',

    descripcionLarga: `El resultado de las 500 Millas de 1930 y la crisis económica que soportaba el país, fueron las razones principales para que un silencio de automotores se impusiera en Rafaela. Durante los años 1931, 1932, 1933 y 1934 no se realizaron ediciones de las "500 Millas". En 1935, una nueva corriente circuló por el Club Atlético a través del impulso de cuatro hombres: Armando Romitelli, Dr. Lapidú Portela, Luis Venanci y quien, sin duda alguna, fue el motor de esta rica historia del Club Atlético, el Ing. Juan R. Báscolo.

La consigna era reeditar las 500 Millas. Había que viajar a Buenos Aires. El viaje se hizo. El Automóvil Club Argentino escuchó el llamado del Club Atlético y por primera vez en la historia del automovilismo, un club del interior recibió el apoyo del organismo rector. Dicha entidad se hizo cargo de los gastos para que dos corredores brasileños fueran invitados a participar de las 500 Millas y estos fueron Francisco "Chico" Landi con Fiat y el barón Manuel de Teffe con Alfa Romeo, agregándose por su cuenta otro piloto brasileño: Cicero Marquez Porto. Y así fue como se reiniciaron las 500 Millas el 8 de septiembre de 1935, en un circuito cerrado de 13.914 metros, con 25 corredores inscriptos y el aporte de corredores extranjeros y la tutela de un entusiasmo renovado en los dirigentes del Club Atlético.
`,

    fotos: [

      { url: 'https://images.unsplash.com/photo-1617814076367-b759c7d7e738?auto=format&fit=crop&w=800&q=80', epigrafe: 'Monoplazas americanos en la recta principal.' },

      { url: 'https://images.unsplash.com/photo-1541348263662-e0626628d0cf?auto=format&fit=crop&w=800&q=80', epigrafe: 'Equipos internacionales trabajando en los boxes de Rafaela.' }

    ]

  },

  {

    id: 'etapa-4',

    fecha:'12 de enero 1941',

    periodo: '1941',

    titulo: 'Primera del Turismo Carretera',

    resumen: 'La llegada de la categoría más antigua del mundo.',

    descripcionLarga: `Año 1941 - Primera del Turismo Carretera:

El 12 de enero de 1941 se realizan las "XII HORAS DE TURISMO CARRETERA". Es las 13ª competencia organizada por la Asociación de Corredores de Turismo Carretera (ACTC) y la primera entre las instituciones que en la actualidad realizan competencias.

También fue la primera competencia de la ACTC disputada en un circuito cerrado de 11.776 metros, ubicada en la continuación del Bv. Roca.

El ganador de la competencia fue Eusebio Marcilla, el Caballero del Camino, que recorrió 1528 km, a un promedio de 126,500 km/h.

12 HORAS de RAFAELA – Resultados FINALES:

1. 9- Eusebio C. Marcilla - Miguel Salem Chev. 12h.01m.09s.4 129 vueltas
2. 4- Domingo Porporato - Mauricio B. Ronchetti Ford 12h.00m.38s.5 126 ''
3. 3- Ernesto H. Blanco - Alberto E. Fava Ford 12h.04m.52s.7 125 ''
4. 8- Héctor A. Barragan - Ignacio Janices Ford 12h.01m.24s.6 123 ''
5. 22- Manuel Montes - Raul Marquez Chev. 12h.04m.33s.0 121 ''
6. 12- Carlos Solveyra Tomkinson - Antonio Evolo Ford 12h.04m.11s.0 117 ''
7. 27- Emilio Karstulovic Bonaci - Raul Cestac Mercury 12h.00m.09s.9 116 ''
8. 11- Pedro Iraizos - Adolfo Trillini Chev. 12h.02m.29s.0 113 ''
9. 7- Jaime B. Ortega - Ricardo Campanilla Ford 12h.04m.44s.0 108 ''
10. 21- Angel L. Pacuali - Florentino F'z. Walker  Ford 12h.01m.48s.0 99 ''
11. 19- Angel Teper - Napoleon Taborda Ford 12h.03m.00s.0 95 ''
12. 15- Antonio A. D. Ochagavia - Domingo Diletti Ford 12h.02m.41s.0 89 ''
Promedio del ganador Eusebio Marcilla 126,5 km/h.`,

    fotos: [

      { url: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&w=800&q=80', epigrafe: 'Turismo Carretera acelerando a fondo en el óvalo asfaltado.' }

    ]

  },

  {

    id: 'etapa-5',
    fecha:'29 de marzo 1947',

    periodo: '1947',

    titulo: 'Primera carrera Internacional en Rafaela',

    resumen: 'Consolidación internacional, con pilotos de europa corriendo en un callejero de empedrado.',

    descripcionLarga: `Coches Especiales y Mecánica Nacional

Esta fue la primera temporada internacional en la República Argentina que se disputaría en: Palermo, Buenos Aires; Parque Independencia, en Rosario y la tercera competencia en el Circuito Parque, frente a la Jefatura de Policía de Rafaela, de 2.145,75 metros de longitud.

Superado el trágico período de la Segunda Guerra Mundial, la actividad automovilística se reinició en todo el país. Rafaela no podía quedar atrás en este despertar. El Club Atlético de Rafaela vuelve por sus fueros, integra comisiones, prepara el clima y se dan los primeros pasos para reeditar las jornadas de gloria de la institución. Se suma a Retiro y Rosario para la primera temporada internacional de Coches Especiales en la República Argentina con la presencia de los pilotos italianos Aquiles Varzi, Luiggi Villoresi, Giacomo Palmieri, Carlos Pintacuda, Victorio Rosa, el francés George Raph, el brasilero Francisco "Chico" Landi y los argentinos: Oscar y Juan Gálvez, Italo Bizio, Pablo L. Pesatti, entre otros.`,

    fotos: [

      { url: 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&w=800&q=80', epigrafe: 'Vistas aéreas del circuito modernizado.' }

    ]

  },

  {

    id: 'etapa-6',

    fecha:'23 de octubre 1949',

    periodo: '1949',

    titulo: 'Trágicas 500 Millas Argentinas',

    resumen: 'El pajarón, simbolo de velocidad y gloria.',

    descripcionLarga: `Un recuerdo muy triste de esta competencia. En la última vuelta de la prueba de clasificación, pierde la vida Adolfo Scandroglio, luego de haber conseguido el mejor tiempo en la vuelta anterior a su fatal accidente. El "PAJARON" estaba equipado con un motor Fiat A-12bis de 21,7 litros y 6 cilindros en línea, capaz de entregar unos 320 HP, rescatado de un hidroavión Caproni,  y se montó sobre el chasis de una antigua limusina Minerva.
    
    Un sincero homenaje de Atlético de Rafaela a todos los pilotos y acompañantes que en distintos escenarios dejaron su vida por su pasión por el automovilismo. 1927: Jorge Cáceres (carrera).
    
    1930: 4 espectadores debido a un despiste de Alberto Fontana (carrera).
    1936: Ricardo Jolly y Lardo Jurevich (entrenamientos).
    1938: Manuel Manrique (carrera).
    1947: Hipólito Bolcatto (carrera).
    1949: Adolfo Scandroglio (entrenamientos), Ítalo Bizio y 10 espectadores del despiste (carrera).`,

    fotos: [

      { url: 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=800&q=80', epigrafe: 'Exhibición de autos históricos en el centenario.' }

    ]

  },

  {

    id: 'etapa-7',

    fecha:'24 de diciembre 1950',

    periodo: '1950',

    titulo: 'Autos de F1 en Rafaela',

    resumen: 'Otra proeza de los directivos de Atlético de Rafaela.',

    descripcionLarga: `Las Talbot Lago en las "500 Millas"
    El Ing. Juan R. Báscolo viaja a Milán, Italia, a los fines de contratar el equipo oficial Talbot para su participación en las "500 Millas". La gestión llega a feliz término y es así que Juan M. Fangio, José F. González y Luis Rosier, propietario del equipo, participan de la competencia en un circuito cerrado de tierra de 8.746,41 metros de recorrido. 
    Se disputaría el Premio Presidente de Nación General "Juan Domingo Perón". Tras suspensiones por lluvia al fin se pudieron cumplir las "500 Millas" el 24 de diciembre de 1950. Era víspera de Navidad y según el periodista Merlo, en el "Mundo Deportivo": "El Club Atlético Rafaela, se ha salido con la suya. Ganó. Y al imponerse victoriosamente su tesis, ha triunfado el automovilismo argentino. ¡La tierra no es el 'asustachico' que se creía! La noble tierra, tratada y cuidada con cariño y dedicación, puede responder con creces. 
    
    Aquel circuito rafaelino, nacido al calor de los caminos vecinales hendido a pata de cabra, cariñosamente asistido por la dedicación de los viejos, enamorados de la mecánica nacional, ha sabido dar esta satisfacción plena. Rafaela, fue en esta semana punto geográfico obligado en todos los titulares de ancho de página de los diarios del mundo entero."
    
    Fue la penúltima edición realizada en el viejo circuito ubicado al oeste de la ciudad, en la prolongación del bulevar Presidente Roca y el luego cinco veces campeón mundial, condujo una de las tres Talbot-Lago T26C traídas desde Francia por Louis Rosier. El equipo a batir, estuvo integrado ese día por Fangio, José Froilán González y el propio Rosier, ejerciendo un claro dominio sobre el resto de los participantes en la clásica prueba rafaelina.

  Fangio señaló el mejor tiempo en clasificación, estableciendo un récord para el escenario de tierra, a más de 207 kilómetros por hora de promedio y terminó quedándose con la victoria en la carrera seguido por Rosier, en tanto que debió abandonar González.

  Era impensado en aquella época que tres máquinas procedentes de la Fórmula 1 Internacional se presenten en una superficie totalmente distinta, pero quedó demostrado que no tuvieron ninguna dificultad para imponer condiciones en la tierra compactada de la Pampa Gringa.

  Clasificación general: 1° Juan Manuel Fangio (Talbot Lago), en 4h32m37s2d para las 92 vueltas; 2° Louis Rosier (Talbot Lago) a 1m22s6d y 3° Luis Brosutti (Mercedes Benz SSK) a 8 vueltas.`,

    fotos: [

      { url: 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d?auto=format&fit=crop&w=800&q=80', epigrafe: 'El autódromo hoy: tecnología y pasión de vanguardia.' }

    ]

  },

  {

    id: 'etapa-8',

    fecha:'2 de agosto 1953',

    periodo: '1953',

    titulo: 'Inauguración del Autódromo "Ciudad de Rafaela"',

    resumen: 'El óvalo cambia de lugar y de tamaño, pero la pasión sigue intacta.',

    descripcionLarga: `Año 1953 - Inauguración del Autódromo "Ciudad de Rafaela":

En el orden del día del llamado a Asamblea Extraordinaria del lunes 23 de febrero de 1953 se destacaba un punto: Informe de la gestión realizada para la adquisición de un terreno destinado al Autódromo, dispuesto por la Asamblea Extraordinaria de 26 de enero pasado y autorización para su compra.   La Asamblea autorizó de pie y por aclamación la adquisición del terreno, el paso más importante en su vida deportiva.   La Comisión de Automovilismo estaba conformada por: Ing. Juan R. Báscolo (presidente), Reinaldo Ripamonti (vicepresidente), Américo Berta (secretario), Ricardo Santi (prosecretario) y Pedro Larroque (tesorero).

Con respecto al terreno que reunía las condiciones adecuadas para la obra, era propiedad de Faustino Ripamonti Ltda., ubicado a 4 kilómetros al norte de la ciudad y con unas dimensiones de 2200 metros de norte a sur y 600 metros de este a oeste (134 hectáreas).

La construcción del circuito la llevó a cabo la Empresa de Calixto Marcos; un óvalo de tierra de 4.662,60 metros, de 12 metros de ancho con dos rectas de 1.477,10 metros y dos curvones de 854,20 metros con un peralte del 7% en sus curvas.

Si se hiciera una ligera comparación con las demás pistas de renombre mundial, llegaríamos a resultados netamente favorables para Rafaela. Por ejemplo, la famosa de Indianápolis tenía en 1953 un recorrido total de 4.020 metros y las rectas llegaban a una longitud de 1.010 metros.

El 2 de agosto de 1953 se inaugura el Autódromo "Ciudad de Rafaela", ubicado en la prolongación del bulevard Lehmann al norte de la Ciudad. La pista era un óvalo de tierra con una extensión de 4.662,60 metros, inaugurada con una competencia de Turismo Carretera, cuyo ganador fue Juan Gálvez. Se llevó el 2º Premio "Ciudad de Rafaela".`,


    fotos: [

      { url: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&w=800&q=80', epigrafe: 'Turismo Carretera acelerando a fondo en el óvalo asfaltado.' }

    ]

  },
  {
    id: 'etapa-9',
    fecha:'23 de mayo 1954',
    periodo: '1954',
    titulo: 'Primera 500 Millas en el nuevo Autódromo "Ciudad de Rafaela"',
    resumen: 'La primera edición de las 500 Millas en el nuevo óvalo de tierra.',
    descripcionLarga: `Después de dos años sin 500 Millas en Rafaela, se corrió en el nuevo Ovalo de tierra de 4.662,60 metros de extensión, resultando ganador Roberto Bonomi con Ferrari a un promedio de 152,8 km/h.
    
    Esto nos decia Juan R. Báscolo, en su libro "Son cosas de las 500 Millas", lo que significaba ese momento...
    
    Ya se pronuncian los días que transforman la fisonomía de una ciudad. Es que la actividad se multiplica y el pueblo empieza a sentir el alma de la fiesta. RAFAELA, dinámica en su actividad fabril y remozada en sus cuadros culturales, se apresta de una manera distinta para recibir el acontecimiento deportivo más trascendental de América Latina. Una nueva edición de las "500 Millas Argentinas", la prueba más gaucha que se corre en el centro mismo de la pampa argentina. Ya tiene su planteo de forma definida y solo se espera que los quijotes del tiempo se encuentren frente a los molinos, pero no para deshacer "entuertos" y "agravios", sino con el fin de obtener el laurel de la victoria que le estará diciendo al vencedor que ya está en la lista de los grandes...

    A pesar del tiempo y sinsabores de cada edición de este espectáculo tradicional en las lides motrices de América, las fuerzas de los hombres se equilibran después de un corto descanso, y de nuevo embalados en la tarea de seguir gestando gloria. El tiempo reconocerá que lo hecho por el grupo desde su iniciación hasta la fecha del balance ha sido titánico, pues no solo trabajaron para el Club Atlético de Rafaela, cuyos colores son los de nuestra patria, sino que lo hicieron por la patria "chica" que fue y es RAFAELA, cuna de nuestros mayores y campo fecundo de nuestros hijos, que con orgullo algún día se referirán a sus antepasados que tuvieron la visión extraordinaria de signar al Club como "DE" Rafaela, y por eso entonces, podemos asegurar que los sinsabores y las satisfacciones tienen una sola bandera: RAFAELA.`, 
    
    fotos: [

      { url: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&w=800&q=80', epigrafe: 'Turismo Carretera acelerando a fondo en el óvalo asfaltado.' }

    ]  
  },

  {
    id: 'etapa-10',
    fecha:'4 de septiembre 1966',
    periodo: '1966',
    titulo: 'Pavimentación del Autódromo "Ciudad de Rafaela"',
    resumen: 'El avance más importante que tuvo el Óvalo y un quiebre para llegar a lo más alto del Automovilsmo.',
    descripcionLarga: ` El Club Atlético de Rafaela, con la presidencia de Bernardo Kuschnir y la secretaría a cargo del Ing. Eduardo Ricotti, inicia las gestiones ante el gobernador de la Provincia de Santa Fe, Dr. Aldo Tessio, a fin de solicitarle el aporte de Vialidad Provincial del estudio y proyecto para la pavimentación del Autódromo "Ciudad de Rafaela".

Con la aprobación del Gobierno de la Provincia, el Ing. Williner de Vialidad Provincial, presenta los planos, un óvalo de 4.624,46 metros de recorrido de 12 metros de ancho, con una pendiente del 15 por ciento en su parte superior en sus curvones.

Aprobado por Asamblea Extraordinaria el estudio y proyecto presentado por Vialidad Provincial, se contrata a la Empresa SAOPIM para la construcción de la obra, la que es inaugurada el 4 de septiembre de 1966, construyéndose también, un circuito alternativo de 2.687,14 metros de recorrido, inaugurado el 14 de abril de 1968.


"...Agosto... La ciudad vive un ritmo distinto, el café, la calle, el bar se convulsiona. Chicos y grandes, como hace tiempo, ya formulan nombres, apreciaciones; algunos con más años sobre la espalda escalonan nombres: desvío Zanetti, campo Godino, Roca, campo Manera, Campo Chiaraviglio, campo Fasoli y otros, que se pierden en la euforia de la premisa del recorrido y de los tiempos. Aquello que fue tierra con pinceladas de oro, al recibir el polvo inconsútil de las carreteras, los postrimeros rayos de sol, enmarcan con nombres que ya son historia en el automovilismo de América, exhalan un profundo suspiro, y todo aquel pasado como si fuera una nube milagrera, se extiende haciéndose azul como su sueño... el AUTODROMO PAVIMENTADO... Y así, en un mes de agosto, como hace 40 años, empieza la nueva mística, la del esfuerzo, la pujanza, el corazón de una ciudad, en la obra maravillosa que es el Autódromo "Ciudad de Rafaela", el que tendrá sus nuevos héroes, su nuevo sueño y sus nuevas esperanzas."

"Son cosas de las 500 Millas"
Juan R. Báscolo, cuando se inauguró el pavimento..`, 
    
    fotos: [

      { url: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&w=800&q=80', epigrafe: 'Turismo Carretera acelerando a fondo en el óvalo asfaltado.' }

    ]  
  },

];

// --- DATA LÍNEA HISTÓRICA ---
export const LINEA_HISTORICA = [
  { year: 1919, important: true, etapaId: 'etapa-1' },
  { year: 1926, important: true, etapaId: 'etapa-2' },
  { year: 1935, important: false, etapaId: 'etapa-3' },
  { year: 1941, important: false, etapaId: 'etapa-4' },
  { year: 1947, important: false, etapaId: 'etapa-5' },
  { year: 1949, important: false, etapaId: 'etapa-6' },
  { year: 1950, important: false, etapaId: 'etapa-7' },
  { year: 1953, important: true, etapaId: 'etapa-8' },
  { year: 1954, important: false, etapaId: 'etapa-9' },
  { year: 1966, important: true, etapaId: 'etapa-10' },
  { year: 1971, important: true, etapaId: 'etapa-11' },
  { year: 1973, important: false, etapaId: 'etapa-12' },
  { year: 1982, important: false, etapaId: 'etapa-13' },
  { year: 1998, important: false, etapaId: 'etapa-14' },
  { year: 2005, important: true, etapaId: 'etapa-15' },
  { year: 2012, important: false, etapaId: 'etapa-16' },
  { year: 2019, important: true, etapaId: 'etapa-17' },
  { year: 2026, important: true, etapaId: 'etapa-18' },
];

export const HOMENAJES = [
  { 
    id: 1, 
    nombre: 'Juan Manuel Fangio', 
    rol: 'Piloto', 
    foto: '/public/images/historia/fama/juan-manuel-fangio.jpg', 
    descripcion: 'El 5 veces campeón de F1, considerado el mejor piloto de la historia del mundo, sin dudas fue la persona más importante que ganó en Rafaela. Lideró las 500 Millas Argentinas en 1950 de punta a punta a bordo de un Talbot-lago F1 Internacional, marcando un récord de velocidad a 207 km/h de promedio de vuelta, que durante años, fue difícil de superar.' 
  },
  { 
    id: 2,
    nombre: 'Lucio Casarín',
    rol: 'Dirigente',
    foto: '/public/images/historia/fama/lucio-casarin.jpg',
    descripcion: 'Arquitecto y dirigente visionario que proyecto muchas obras en el Autódromo "Ciudad de Rafaela". Lamentablemente perdió la vida en un trágico accidente en la recta principal en el año 1985, pero su legado perdura en nuestra memoria.' 
  },
  { 
    id: 3, 
    nombre: 'Jorge Ternengo', 
    rol: 'Piloto', 
    foto: '/public/images/historia/fama/jorge-ternengo.jpg', 
    descripcion: 'Ídolo rafaelino. Representó a la ciudad en las categorías más grandes del país, siendo profeta en su propia tierra, al ganar las 500 Millas Argentinas en 1969 a bordo de un Bravi-Tornado.' 
  },
  { 
    id: 4, 
    nombre: 'Eduardo Ricotti', 
    rol: 'Dirigente', 
    foto: '/public/images/historia/fama/eduardo-ricotti.png', 
    descripcion: 'Ingeniero y dirigente del club, fue el alma máter del autódromo durante los años de mayor crecimiento en cuanto a obras y seguridad del circuito. Como presidente del club, su visión audaz y su compromiso fue clave para lograr la carrera más importante que Rafaela y Santa Fe tuvieron en toda su historia, la hazaña de traer IndyCar en 1971.' 
  },
  {
    id: 5,
    nombre: 'Juan María Traverso',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/juan-traverso.jpg',
    descripcion: 'El "Flaco", uno de los mejores pilotos que dió Argentina. Es el máximo ganador en Rafaela a nivel nacional, cosechando 7 victorias históricas en el óvalo.'
  },
  { 
    id: 6, 
    nombre: 'Leonelo Bellezze', 
    rol: 'Periodista', 
    foto: '/public/images/historia/fama/leonelo-bellezze.webp', 
    descripcion: 'La voz del automovilismo en Rafaela. Tuvo el histórico honor de transmitir al mundo la mítica carrera de las 300 Millas Indy en 1971 a través de LT28 Radio Rafaela.' 
  },
  {
    id: 7,
    nombre: 'Juan R. Báscolo',
    rol: 'Dirigente',
    foto: '/public/images/historia/fama/juan-bascolo.jpeg',
    descripcion: 'Visionario del automovilismo. Su liderazgo y pasión por las 500 Millas fueron fundamentales para la inauguración del autódromo y la consolidación de Rafaela como un referente en el automovilismo argentino.'
  },
  {
    id: 8,
    nombre: 'Ero Borgogno',
    rol: 'Dirigente',
    foto: '/public/images/historia/fama/ero-borgogno.jpg',
    descripcion: 'Para "Chispa", el autódromo era su segundo hogar. Fue un pilar histórico de la Subcomisión de Automovilismo de Atlético de Rafaela durante 45 años. En su honor, hoy un paseo con su nombre recorre el camino de la ciudad hacia el autódromo.'
  },
  {
    id: 9,
    nombre: 'Al Unser',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/al-unser.jpg',
    descripcion: 'Leyenda norteamericana. El 28 de febrero de 1971, dominó y ganó las míticas 300 Indy en Rafaela a más de 320 km/h, convirtiendo al Autódromo "Ciudad de Rafaela" en el más VELOZ del mundo. Fue la primer vez que IndyCar disputó una carrera oficial fuera de EEUU.'
  },
  {
    id: 10,
    nombre: 'Bernardo Kuschnir',
    rol: 'Dirigente',
    foto: '/public/images/historia/fama/bernardo-kuschnir.jpg',
    descripcion: 'Dirigente fundamental de Atlético de Rafaela y la Sub-Comisión de Automovilismo. Su trabajo institucional fue clave para mantener al club y al autódromo en lo más alto del deporte nacional.'
  },
  {
    id: 11,
    nombre: 'Ruben Luis Di Palma',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/ruben-dipalma.jpg',
    descripcion: 'El "Loco" de Arrecifes, ídolo del automovilismo nacional, fue uno de los grandes protagonistas que desafió el peralte de Rafaela. Ganó la última edición de las 500 Millas Argentinas en 1975, aportando su manejo agresivo y espectacularidad.'
  },
  {
    id: 12,
    nombre: 'Mariano Werner',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/mariano-werner.jpg',
    descripcion: 'Multicampeón del TC moderno que ha protagonizado definiciones infartantes en el óvalo, uno de los favoritos de la nueva generación de fans.'
  },
  {
    id: 13,
    nombre: 'René Zanatta',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/rene-zanatta.jpeg',
    descripcion: 'Héroe local que marcó un hito en su propia tierra. Además de sus 7 victorias en carreras de auto y moto, rompió el récord sudamericano de velocidad en moto, alcanzando los 309,554 km/h.'
  },
  {
    id: 14,
    nombre: 'Lloyd Ruby',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/lloyd-ruby.jpg',
    descripcion: 'Hizo historia en 1971 al marcar la pole position de la 300 Indy en el óvalo de Rafaela, logrando un tiempo impresionante de 59´74´´ a 278,675 km/h con su Mongoose Ford Turbo (récord aún vigente) .'
  },
  {
    id: 15,
    nombre: 'Facundo Ardusso',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/facundo-ardusso.jpg',
    descripcion: 'Uno de los pilotos más queridos del TC y fiel representante del automovilismo santafesino, con victorias en TC2000, y récord sudamericano de velocidad promedio de vuelta a 264,796 km/h en el Templo de la Velocidad.'
  },
  {
    id: 16,
    nombre: 'Agustín Canapino',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/agustin-canapino.jpg',
    descripcion: 'Múltiple campeón, se consagró corriendo en IndyCar, y que demostró su talento absoluto en Rafaela, logrando  varias victorias  en Súper TC2000 y en el Turismo Carretera.'
  },
  {
    id: 17,
    nombre: 'Gabriel Ponce de León',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/gabriel-poncedeleon.jpg',
    descripcion: 'Entró en la historia del óvalo en 2005 al ganar la épica carrera de TC2000, última en el óvalo sin chicanas. En 2012 marcó el récord sudamericano de velocidad final alcanzando 306,383 km/h.'
  },
  {
    id: 18,
    nombre: 'Luis Sojit',
    rol: 'Periodista',
    foto: '/public/images/historia/fama/luis-sojit.jpg',
    descripcion: 'Persona fundamental para que las 500 Millas Argentinas sean posibles. Su voz inconfundible inmortalizó las épicas definiciones de las primeras grandes carreras de Rafaela para todo el país a través de la radio.'
  },
  {
    id: 19,
    nombre: 'Oberdán Piovano',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/oberdan-piovano.jpg',
    descripcion: 'Pionero absoluto. Fue el ganador y uno de los siete valientes que se animaron a largar la histórica primera carrera de automóviles en Rafaela.'
  },
  {
    id: 20,
    nombre: 'Raúl Riganti',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/raul-riganti.jpg',
    descripcion: '"Polenta" fue un referente de los primeros años heroicos del automovilismo, ganador de la primera edición de las 500 Millas Argentinas.'
  },
  {
    id: 21,
    nombre: 'Oscar y Juan Gálvez',
    rol: 'Pilotos',
    foto: '/public/images/historia/fama/oscar-juan-galvez.jpg',
    descripcion: 'Leyendas de LA  ACTC. Entre los dos, consiguieron 14 campeonatos. Juan con 2 victorias en Rafaela, triunfando en el estreno de 1953 y repitiendo la hazaña en 1955.'
  },
  {
    id: 22,
    nombre: 'Adolfo Scandroglio',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/adolfo-scandroglio.jpg',
    descripcion: 'Protagonista de las 500 Millas, piloteando un auto armado con un motor de avión 22 L., que marcaba los mejores tiempos. Su nombre quedó grabado entre los valientes que desafiaban la velocidad en Rafaela.'
  },
  {
    id: 23,
    nombre: 'José Fanto',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/jose-fanto.jpg',
    descripcion: 'Piloto rafaelino que ganó las 500 de 1949 y 1951, donde Rafaela ya era considerada la Cumbre del Automovilismo Nacional.'
  },
  {
    id: 24,
    nombre: 'Jorge Cupeiro',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/jorge-cupeiro.jpg',
    descripcion: 'Dominó en Rafaela durante las décadas del 60 y 70, logrando 3 grandes victorias y brillando especialmente al mando del revolucionario Chevitú.'
  },
  {
    id: 25,
    nombre: 'José Froilán González',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/jose-froilan-gonzalez.jpg',
    descripcion: 'El subcampeón del mundo de F1 engalanó a Rafaela con su presencia y talento en competencias de fuerza libre en las 500 Millas. Fue ganador del Trofeo Challenger Rafaelino, antes de conquistar Europa.'
  },
  {
    id: 26,
    nombre: 'Eusebio Marcilla',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/eusebio-marcilla.jpg',
    descripcion: 'El "Caballero del Camino". Hizo historia el 12 de enero de 1941 al ganar las extenuantes "12 Horas de Rafaela", primera visita del TC a la ciudad, y la primera carrera del TC en un circuito cerrado, fuera de las rutas.'
  },
  {
    id: 27,
    nombre: 'Carlos Reutemann',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/carlos-reutemann.jpg',
    descripcion: 'El gran ídolo santafesino. Lole brilló y se fogueó en los circuitos locales, probando su enorme talento conductivo frente a la afición rafaelina antes de su consagración en Fórmula 1. Es memorable la carrera de empate técnico con Omar Franco, llegando los dos autos a la par, en el Turismo Anexo "J", hoy Turismo Nacional.'
  },
  {
    id: 28,
    nombre: 'Oreste Berta',
    rol: 'Chasista',
     foto: '/public/images/historia/fama/oreste-berta.webp',
    descripcion: 'Una de las personas mas influyentes en el deporte motor argentino, el "Mago", nacido en Rafaela, los potentes motores y chasis preparados por él dominaron innumerables veces el asfalto de Rafaela en manos de los mejores pilotos del país.'
  },
  {
    id: 29,
    nombre: 'Domingo y Clemar Bucci',
    rol: 'Piloto',
     foto: '/public/images/historia/fama/clemar-bucci.jpg',
    descripcion: 'Representantes de una de las dinastías más importantes del automovilismo zonal. Su pasión y velocidad son parte de los cimientos históricos del club.'
  },
  {
    id: 30,
    nombre: 'Carlos Zatuszek',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/carlos-zatuzek.jpg',
    descripcion: 'Volante histórico de la vieja escuela. Sus épicas batallas en las 500 Millas Argentinas lo convirtieron en un referente del circuito en las décadas de 1920 y 1930.'
  },
  {
    id: 31,
    nombre: 'Armando Romitelli',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/armando-romitelli.jpg',
    descripcion: 'Destacado piloto rafaelino, dejando su huella estampada en la gloria en 1921.'
  },
  {
    id: 32,
    nombre: 'Néstor García Veiga',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/nestor-garcia-veiga.jpg',
    descripcion: 'Un piloto de gran jerarquía que deslumbró al público rafaelino por su velocidad, marcando un récord sudamericano promedio de 249,222 km/h en el año 1973. Esta marca fue imbatible hasta el record del SuperTC2000 en el 2012.'
  },
  {
    id: 33,
    nombre: 'Carlos Pairetti',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/carlos-pairetti.png',
    descripcion: 'El creador del famoso "Trueno Naranja". No solo compitió, sino que también fue el único piloto Argentino en la carrera de Indy en Rafaela.'
  },
  {
    id: 34,
    nombre: 'Eric Forrest Greene',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/eric-forrest-greene.jpg',
    descripcion: 'Un caballero inglés de las pistas argentinas que supo acelerar y dejar su clase en las rudas competencias de la época fundacional de Rafaela, ganando en 1928.'
  },
  {
    id: 35,
    nombre: 'Omar Martínez',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/omar-martinez.jpg',
    descripcion: 'El "Gurí". Ídolo indiscutido de Ford y bicampeón que ha levantado a las multitudes en el óvalo rafaelino.'
  },
  {
    id: 36,
    nombre: 'Guillermo Ortelli',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/guillermo-ortelli.jpg',
    descripcion: 'El Rey de Salto y máximo referente de Chevrolet. Hizo historia en el TC al lograr victorias memorables en el óvalo de Rafaela en 2003 y 2005.'
  },
  {
    id: 37,
    nombre: 'Norberto Fontana',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/norberto-fontana.webp',
    descripcion: 'El ex Fórmula 1 que dejó su sello en el asfalto rafaelino, logrando importantes triunfos, como su recordada victoria en el Súper TC2000 en 2014.'
  },
  {
    id: 38,
    nombre: 'Matías Rossi',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/matias-rossi.jpg',
    descripcion: 'El "Misil". Siempre protagonista en Rafaela, destacado por su precisión conductiva, fue la estrella del 2005, que sin haber ganado, cautivo al público doblando en los curvones al raz del paredón a mas de 200 km/h, hazaña que lo llevó al podio en aquella histórica carrera de TC2000 sin chicanas.'
  },
  {
    id: 39,
    nombre: 'Martín Basso',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/martin-basso.webp',
    descripcion: 'Piloto local que llevó la bandera de la ciudad a lo más alto en competencias nacionales y sudamericanas, representando el orgullo y la herencia fierrera de Rafaela.'
  },
  {
    id: 40,
    nombre: 'Nicolás González',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/nicolas-gonzalez.jpeg',
    descripcion: 'Referente contemporáneo del automovilismo rafaelino, fue el último rafaelino en correr en el TC, acelerando frente a su gente y manteniendo viva la pasión local en las categorías de la ACTC.'
  },
   {
    id: 41,
    nombre: 'Christian Ledesma',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/christian-ledesma.jpg',
    descripcion: 'Piloto histórico del TC con Chevrolet. Christian, llegó a lo más alto del Automovilismo Nacional, conquistando el Autódromo Ciudad de Rafaela en 2 ocaciones.'
  },
  {
     id: 42,
    nombre: 'Miguel Ángel Guerra',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/miguel-guerra.jpg',
    descripcion:'Uno de los pilotos que mejor representó a Argentina en la F1, fue el que mejor supo entender el circuito, ganando importantes carreras.'
  },
  {
    id: 43,
    nombre: 'Fabián Acuña',
    rol: 'Piloto',
    foto: '/public/images/historia/fama/fabian-acuña.jpg',
    descripcion: 'Referente del TC que supo conquistar 3 veces el óvalo rafaelino, con Ford y Chevrolet.'
  }
];

export const CIRCUITOS = [
  { 
    id: 'circuito-1', 
    nombre: 'El Trazado Original de 1919', 
    medida: '320 km', 
    epoca: '1919', 
    foto: '/public/images/circuito 1919 .png', 
    detalle: 'Formado por caminos de tierra que unían Rafaela con localidades vecinas. Aquí nació la pasión por la velocidad en la región, con los primeros rugidos de motores a más de 100 km/h.' 
  },
  { 
    id: 'circuito-2', 
    nombre: 'El Óvalo de 4 curvas', 
    medida: '1° Edición: 38.0000 metros - Última Edición: 8.746 metros', 
    epoca: '1926 - 1951', 
    foto: '/public/images/Circuito 500 Millas.jpg', 
    detalle: 'La medida fue cambiando con el paso del tiempo. Comenzó en 1926 con 38.000 metros, reduciéndose a 11.776,55 metros en la decada del ´30, para terminar en el año 1951 de 8.746 metros. Los distintos formatos siempre tuvieron la misma esencia, una mística que lo hacía único y un adjetivo en común que lo definían, VELOZ.' 
  },
  { 
    id: 'circuito-3', 
    nombre: 'El Óvalo peraltado de tierra', 
    medida: '4.662.60 metros', 
    epoca: '1953-1966', 
    foto: '/public/images/autodromo 1953.png', 
    detalle: 'El óvalo de tierra de 4.662,60 metros, de 12 metros de ancho con dos rectas de 1.477,10 metros y dos curvones de 854,20 metros con un peralte del 7% en sus curvas. Fue inaugurado el 2 de agosto de 1953 con una competencia de Turismo Carretera.' 
  },
  { 
    id: 'circuito-4', 
    nombre: 'El Óvalo peraltado de asfalto', 
    medida: '4.624.46 metros', 
    epoca: '1966-Actualidad', 
    foto: '/public/images/circuito 1966.png', 
    detalle: 'Constituido por el circuito N°1, conocido como el Óvalo Rafaelino, y por el circuito N° 2. El óvalo de asfalto tiene un recorrido de 4.624,46 metros, mientras que el circuito interno es de 2.687,14 metros. El óvalo, que en principio fue de 12 metros de ancho, hasta la extensión a 18 metros en el año 1971, contaba con 2 rectas de 1.181,01 metros y dos curvones especialmente diseñados, conformados por: 2 curvas clotoides de 361,52 m  y una curva circular de 408,18 m. Con un peralte máximo del 15% (8.5°). Máximo radio en curvas de 245 metros. Fue inaugurado el 4 de septiembre de 1966 con una competencia de 500 Millas, ganador Jorge Cupeiro. Récord de velocidad promedio: Lloyd Ruby a 278,675 km/h a 59´ 7´´.' 
  },
  {
    id: 'circuito-5',
    nombre: 'Circuito N°2 "Ing. Juan R. Báscolo"',
    medida: '3.050 metros',
    epoca: '1982-Actualidad',
    foto: 'public/images/circuito 2 bascolo.jpg',
    detalle: 'Es un circuito mixto de 3.050 metros, alternativo al óvalo, compartiendo el curvón norte. Con un ancho de 18 metros, cuenta con una recta principal de 710 metros y 7 curvas. Inicialmente su sentido fue horario hasta el año 2000, cambiando a sentido antihorario en la actualidad. Fue inaugurado el 26 de septiembre de 1982 con una competencia de CAP-Datsun 280, resultando ganador Guillermo Kissling.'
  },
  {
    id: 'circuito-6',
    nombre: 'El Óvalo con 4 chicanas"',
    medida: '4.780 metros',
    epoca: 'En desuso',
    foto: '/public/images/trazado 2013.png',
    detalle: 'Este óvalo con 4 chicanas se utilizó solamente en abril y octubre del año 2000. Las 2 ocaciones fue con el Turismo Carretera, resultando ganadores Fabián Acuña y Marcos Di Palma.'
  },
  {
    id: 'circuito-7',
    nombre: 'El Óvalo con 2 o 3 chicanas',
    medida: '4.740 metros',
    epoca: 'Actualidad',
    foto: '/public/images/circuito 3d 2026.png',
    detalle: 'Este óvalo con 2 o 3 chicanas es el que usa actualmente el TC y el TC2000, añadiendo una capa adicional de complejidad y frenados a las carreras.'
  }
];