...
import { QrCode, Download, Ticket, AlertTriangle, ChevronLeft, Music } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchEventos = async () => {
            try {
                ...
                // Filtramos solo los recitales
                const recitales = res.data.filter((e: any) => e.tipo_evento === 'Recital');
                setEventos(recitales); 
            } catch (error) {
                console.error("Error cargando recitales:", error);
            }
        };
        fetchEventos();
    }, []);
                ...
                {paso === 1 ? (
                    <div className="relative z-10 flex flex-col items-center py-8">
                        ...
                        <button 
                            ...
                            className="bg-purple-500 hover:bg-purple-600 text-white font-extrabold text-xl py-4 px-8 rounded-2xl transition-all flex items-center justify-center gap-3 w-full sm:w-auto disabled:opacity-50"
                        >
                            <QrCode size={28} />
