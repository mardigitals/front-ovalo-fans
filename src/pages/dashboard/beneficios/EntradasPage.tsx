...
import { QrCode, Download, Share2, Ticket, AlertTriangle, ChevronLeft } from 'lucide-react';
...
const EntradasPage = () => {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                const carreras = resEventos.data.filter((e: any) => e.tipo_evento === 'Carrera');
                setEventos(carreras); 

                const resHistorial = await api.get('/uso-beneficio/mis-usos');
                const misEntradas = resHistorial.data.filter((u: any) => u.tipo_beneficio === 'DESC_CARRERAS');
                setHistorial(misEntradas);
            } catch (error) {
    ...
    useEffect(() => {
        if (eventoSeleccionado) {
            ...
            if (qrExistente) {
                setQrData(qrExistente.codigo_qr || `DESC-CARRERA-${qrExistente.id}-${userProfile?.id}`);
                setPaso(2);
    ...
    const descargarQR = async () => {
        try {
            ...
            link.href = url;
            link.download = `VoucherCarrera-OvaloFans-${userProfile?.alias || 'Socio'}.png`;
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    ...
                    text: 'Acá está mi código QR de descuento para la carrera en el Autódromo.',
                    url: qrImageUrl,
                });
            } catch (error) {
                ...
                {paso === 1 ? (
                    <div className="relative z-10 flex flex-col items-center py-8">
                        ...
                        <button 
                            ...
                            className="bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-xl py-4 px-8 rounded-2xl transition-all flex items-center justify-center gap-3 w-full sm:w-auto disabled:opacity-50 shadow-[0_0_30px_-5px] shadow-orange-500/50"
                        >
                            <QrCode size={28} />
                    ...
                    <div className="relative z-10 flex flex-col items-center animate-in zoom-in duration-500">
                        ...
                        <button 
                            onClick={() => setPaso(1)}
                            className="mt-6 text-sm font-bold text-slate-500 hover:text-orange-500 transition-colors underline"
                        >
